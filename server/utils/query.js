...
const sqlContent = fs.readFileSync(path.resolve(__dirname,'..','./sql/zjy_blog.sql'),'utf-8')
let pool
...
init.connect()
init.query('CREATE DATABASE zjy_blog',err=>{
    ...
    if(err){
        console.log('zjy_blog database created already')
    }else{
        console.log('create zjy_blog Database')
